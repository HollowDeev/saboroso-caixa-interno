...
export default function DiscountsPage() {
  ...
  // Simulação de fetch dos descontos (substitua pelo service real)
  useEffect(() => {
    setLoading(false);
...
// Modal de desconto
function DiscountModal({ open, onClose, onSave, editing, products, externalProducts }: any) {
  ...
  // Busca produto/comida selecionado
  const item = productType === "food"
    ? products.find((p: any) => p.id === productId)
    : externalProducts.find((p: any) => p.id === productId);
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSave} className="space-y-3">
          ...
          >
            ...
            {(productType === "food" ? products : externalProducts).map((p: any) => (
