...
export const CashRegisters = () => {
  ...
  const [loadingRegister, setLoadingRegister] = useState<Record<string, boolean>>({});
  const [openSalesByRegister, setOpenSalesByRegister] = useState<Record<string, string[]>>({});
  ...
  const handleAccordionChange = async (registerId: string) => {
    ...
    if (!salesByRegister[registerId] || !expensesByRegister[registerId]) {
      setLoadingRegister((prev) => ({ ...prev, [registerId]: true }));
      // Buscar vendas
      ...
      setSalesByRegister((prev) => ({ ...prev, [registerId]: formatSales(salesParsed) }));
      // Buscar despesas
      const { data: expenses } = await supabase
        ...
        .eq('cash_register_id', registerId)
        .order('created_at', { ascending: false });
      setExpensesByRegister((prev) => ({ ...prev, [registerId]: expenses ? formatExpenses(expenses) : [] }));
      setLoadingRegister((prev) => ({ ...prev, [registerId]: false }));
    }
  };

  const toggleSale = (registerId: string, saleId: string) => {
    setOpenSalesByRegister((prev) => {
      const openSales = prev[registerId] || [];
      return {
        ...prev,
        [registerId]: openSales.includes(saleId)
  ...
  const filteredSales = (sales: Sale[] | undefined) => {
    if (!sales) return [];
    return "all" // selectedPaymentMethod === "all"
      ? sales
      : sales.filter(sale => sale.payments.some(p => p.method === "all")); // selectedPaymentMethod === "all"
  };
  ...
  return (
    <div className="p-0 md:p-4 lg:p-6 space-y-4 md:space-y-6">
      ...
      <div className="grid gap-4">
        <Accordion type="multiple" className="w-full" value={openRegisters} onValueChange={(ids) => {
          // Accordion do radix retorna array de ids abertos
          if (Array.isArray(ids)) {
            const newId = ids.find(id => !openRegisters.includes(id));
            if (newId) handleAccordionChange(newId);
            setOpenRegisters(ids);
          }
        }}>
          {cashRegisters.map((register) => (
            <AccordionItem key={register.id} value={register.id} className="bg-white border-x-0 md:border-x md:rounded-lg mb-4 first:border-t-0 md:first:border-t last:border-b-0 md:last:border-b">
              ...
              <AccordionContent className="px-4 pb-4">
                {loadingRegister[register.id] ? (
                  <div className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Carregando vendas e despesas...</div>
                ) : (
                  <>
                        ...
                        <TabsContent value="sales" className="px-4 md:px-0">
                          ...
                          <div className="space-y-3">
                            {filteredSales(salesByRegister[register.id])?.map((sale) => {
                              const isOpen = (openSalesByRegister[register.id] || []).includes(sale.id);
                              return (
                                <div
                                  key={sale.id}
                                  className="bg-gray-50 rounded-lg shadow-sm"
                                >
                                  <div className="p-4 cursor-pointer" onClick={() => toggleSale(register.id, sale.id)}>
